...
$.extend($.validator, {
	...
	messages: {
		...
		rangelengths: $.validator.format("Please enter a value between {0} and {1} characters longs."),
	...
	classRuleSettings: {
        ...
        isSelectFile: {isSelectFile: true}
	},
	...
	normalizeRules: function(rules, element) {
		...
		$.each(['rangelength','rangelengths', 'range'], function() {
			if (rules[this]) {
		...
		if ($.validator.autoCreateRanges) {
			...
			if (rules.minlength && rules.maxlength) {
				...
				rules.rangelengths = [rules.minlength, rules.maxlength];
	...
	methods: {
		...
		// http://docs.jquery.com/Plugins/Validation/Methods/rangelength
		rangelengths: function(value, element, param) {
			var length = this.getLength($.trim(value), element);
        ...
        // http://docs.jquery.com/Plugins/Validation/Methods/telephone
		telephone: function(value, element) {
			// contributed by Scott Gonzalez: http://projects.scottsplayground.com/email_address_validation/
			return this.optional(element) || /^(\+86|0)?((13[0-9])|(14[5|7])|(15[^4,\D])|(18[^4,\D]))\d{8}$/i.test(value);
		},
		fileType: function(value, element) {
			// contributed by Scott Gonzalez: http://projects.scottsplayground.com/email_address_validation/
			var values = value.substring(value.lastIndexOf("."),value.length),
            	value = values.toLowerCase();
			return this.optional(element) || /^(.jpg|.gif|.png|.bmp|.doc|.xls|.rar|.zip)$/i.test(value);
		},
		isSelectFile: function(value, element) {
			// contributed by Scott Gonzalez: http://projects.scottsplayground.com/email_address_validation/
			var values = value.substring(value.lastIndexOf("."),value.length),
            	value = values.toLowerCase();
			return this.optional(element) || /^(.txt|.xls|.xlsx)$/i.test(value);
		}
	}
